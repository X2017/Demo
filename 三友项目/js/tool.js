...
(function () {
	...
	(s = ua.match(/msie\s([\d\.]+)/)) ? sys.msie=s[1] : 0;// < IE8
...
function iframeHeight(target) {
    var iframe= $().getId(target);
    iframe.height=document.documentElement.clientHeight;
}
//图片左右移动
function move(obj,attr,target) {
	clearInterval(obj.bottomTimer); //定时器设置
	obj.bottomTimer=setInterval(function () {
		var cur=0;
		if (attr == 'opacity') {
			cur=Math.round(parseInt(obj,attr)*100);
		}else {
			cur=parseInt(getStyle(obj,attr));
		}
		var steep=(target-cur)/6;
			steep= steep > 0 ? Math.ceil(steep) : Math.floor(steep);
		if (target == cur) {
			clearInterval(obj.bottomTimer);
		}else if (attr == 'opacity') {
			obj.style.opacity=(cur+steep)/100;
		}else {
			obj.style[attr]=cur+steep+'px';
		}
	},20);
}
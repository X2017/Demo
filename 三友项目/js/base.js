...
Base.prototype.center = function (width, height) {
	...
	for (var i = 0; i < this.elements.length; i ++) {
		...
		this.elements[i].style.zIndex = 999999;
...
Base.prototype.lock = function () {
	for (var i = 0; i < this.elements.length; i ++) {
		...
		this.elements[i].style.background = 'black';
		this.elements[i].style.zIndex = 9998;
		$(this.elements[i]).opacity(70);
		this.elements[i].style.display = 'block';
...
Base.prototype.animate = function (obj) {
	for (var i = 0; i < this.elements.length; i ++) {
		...
		var time = obj['time'] != undefined ? obj['time'] : 30;												//可选，默认10毫秒执行一次
			...
			for (var i in mul) {
				...
				if (attr == 'opacity') {
					...
					if (step == 0) {
						...
						// $(element).css(attr,parseInt(getStyle(element, attr)) + step + 'px');
					}
